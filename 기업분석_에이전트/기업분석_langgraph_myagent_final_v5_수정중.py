...
import naver_latest_news_urls_v1 as newsurl
...
# 사용자가 중간에 분석 자체를 취소햇을 때 흐름을 빠져나오기 위한 예외
# (그래프 노드 안에서 raise 하면 agent_run.invoke() 호출부까지 그대로 전파됨)

class AnalysisAborted(Exception):
    """사용자가 회사 확인/초안 검토 단계에서 분석을 취소했을 때 발생"""
    pass
...
class CompanyState(TypedDict):
    ...
    price_df: Optional[List[float]]     # (수정) DataFrame이 아니라 종가(Close) float 리스트 — 체크포인터 직렬화 문제 방지
    ...
    draft_feedback : Optional[str]       # (신규) 사용자가 초안에 대해 남긴 수정 요청
    review_attempts : Optional[int]      # (신규) 초안 재작성 시도 횟수 (무한 루프 방지)
...
def find_stock_codes(df: pd.DataFrame, query: str, cutoff: int = 75, top_n: int = 5):
    """회사명(한/영) 유사도 검색 → 상장사만 반환 (상위 top_n개 후보를 그대로 반환)

    * 기존에는 최고 점수 1개만을 반환하기 때문에 비슷한 이름의 다른 회사가 자동으로 확정되어버리는 문제가 있음
    * 이제는 후보를 여러 개 반환하고, 최종 확정은 confirm_company_candidate()에서 사용자가 직접 선택하도록 함
    ...
    return cand[["corp_name", "corp_name_eng", "corp_code", "stock_code", "score"]][:top_n]

# 사용자 확인 코드 설계
def confirm_company_candidate(candidates_df: pd.DataFrame, query: str):
    """
    (신규) 검색 단계 사용자 확인.

    검색된 후보 회사 목록을 사용자에게 직접 보여주고 선택하게 함으로써,
     - 존재하지 않는 회사명을 입력했을 때 조용히 엉뚱한 결과로 진행되는 문제
     - 비슷한 이름의 다른 회사가 자동으로 잘못 확정되는 문제를 막음

    반환값:
        dict -> 사용자가 선택한 후보(해당 후보로 티커 확정 절차 진행)
        None -> 후보가 없거나, 사용자가 "회사명 힌트만으로 진행"을 선택
    예외:
        AnalysisAborted -> 사용자가 분석 자체를 취소한 경우
    """

    if candidates_df.empty:
        print(f"\n[알림] '{query}' 와(과) 일치하는 국내 상장회사를 DART 목록에서 찾지 못했습니다.")
        answer = input("그래도 회사명 힌트만으로 분석을 계속 진행할까요? (y/n): ").strip().lower()
        if answer == "y":
            return None
        raise AnalysisAborted(f"'{query}' 회사를 찾지 못해 사용자가 분석을 취소했습니다.")

    print(f"\n[알림] '{query}'와(과) 유사한 상장회사를 {len(candidates_df)}개 찾았습니다.")

    rows = candidates_df.to_dict(orient="records")
    for i, row in enumerate(rows, 1):
        eng = row.get("corp_name_eng") or "-"
        print(f"{i}) {row['corp_name']} ({eng}) stock_code={row['stock_code']} 유사도={row['score']:.1f}")
    print(" 0) 목록에 없음 / 회사명 힌트만으로 진행")

    while True:
        sel = input("분석하려는 회사 번호를 선택하세요: ").strip()
        if sel == "0":
            return None
        if sel.isdigit() and 1 <= int(sel) <= len(rows):
            return rows[int(sel) - 1]
        print("잘못된 입력입니다. 목록에 있는 번호(0 포함)를 입력해주세요.")
...
def Decision_Company(state: CompanyState):
    ...
    print(f"\n[검색어] {com_hint}")
    candidates = find_stock_codes(df, com_hint)  # (수정) 상위 후보 여러 개 반환

    # ---- (신규) 사용자 확인 단계 ----
    # 후보가 없거나 여러 개일 때 자동으로 하나를 확정 짓지 않고,
    # 사용자가 직접 목록에서 고르거나 진행 여부를 결정하게 한다.
    selected = confirm_company_candidate(candidates, com_hint)

    if selected is None:
        # 사용자가 "회사명 힌트만으로 진행"을 선택한 경우 (또는 애초에 후보가 없던 경우)
        add_note(state, "[Decison Company Node] 사용자가 회사명 힌트 기반 진행을 선택함(티커 없음)")
        state['company_name'] = com_hint
        return state

    # 사용자가 확정한 후보로 티커/시장정보 확인
    stock_code_suffix = detect_market_suffix_naver(selected["stock_code"])
    ticker_name = selected["stock_code"] + stock_code_suffix
    ...
    if info and "shortName" in info:
        ...
        add_note(state, f"[Decison Company Node] 사용자 확인 완료. 티커 확정: {ticker_name} / {state['company_name']}")
        return state

    # yfinance 매핑까지는 실패했지만, 사용자가 고른 회사명은 신뢰할 수 있으므로 그대로 사용
    add_note(state, "[Decison Company Node] yfinance 티커 매핑 실패—사용자가 선택한 회사명 기반 진행")
    state["company_name"] = selected["corp_name"]
    return state
...
def Get_MarketPrice(state: CompanyState):
    ...
    # (수정) DataFrame을 그대로 state에 저장하면 체크포인터(웹/Streamlit 모드)가
    # 상태를 직렬화(msgpack)하는 과정에서 TypeError가 남 (DataFrame은 직렬화 불가).
    # 실제로 아래 Draft_Report에서 쓰는 건 최근 종가(Close)뿐이라, 순수 float 리스트로만 저장.
    price_tail = history.tail(120) if not history.empty else pd.DataFrame()
    state['price_df'] = [float(x) for x in price_tail['Close'].tolist()] if not price_tail.empty else []
...
def Get_NewsData(state: CompanyState):
    ...
    news_query_comname = state.get("company_hint")
    print('news_query_comname : ', news_query_comname)
    if not news_query_comname:
        add_note(state, "[NewsData] 뉴스요청 없음_뉴스 생략")
        return state

    re_query = news_query_comname + " 실적"
    add_note(state, f"[NewsData] Naver 뉴스 검색 : {re_query}")


    normalized_urls = newsurl.extract_article_links(re_query)

    print('naver new url : ')
    print(normalized_urls)

    if normalized_urls == []:
        print("[WARN] naver news container not found | query={}".format(re_query))
        print("Replace with Tavily Search!!")

        normalized_urls = ts.tavily_search(news_query_comname)
        print('tavily new url : ')
        print(normalized_urls)
...
def Draft_Report(state: CompanyState):
    # (수정) price_df는 이제 DataFrame이 아니라 종가(Close) float 리스트
    price_df = state.get('price_df')
    if price_df:
        recent_price = [round(x, 2) for x in price_df[-5:]]
    else:
        recent_price = []
    ...
    # (신규) 사용자가 이전 검토 단계에서 남긴 피드백이 있으면 프롬프트에 반영
    feedback = state.get('draft_feedback')
    if feedback:
        revision_note = (
            f"\n* 이전 초안에 대해 사용자가 다음과 같은 수정 요청을 남겼습니다. "
            f"반드시 반영해서 다시 작성하세요: \"{feedback}\"\n"
        )
    else:
        revision_note = ""
...
# (신규) 최대 재작성 허용 횟수 - 사용자가 계속 "다시 써줘"를 눌러도 무한루프에 빠지지 않도록 제한
MAX_REVIEW_ATTEMPTS = 2

# (신규) 초안 검토 노드: 최종 보고서를 넘어가기 전에 사용자가 직접 초안을 읽고 확정/재작성/취소를 선택
def Review_Draft(state: CompanyState):
    ...
    if attempts >= MAX_REVIEW_ATTEMPTS:
        print(f"[알림] 재작성 요청 한도({MAX_REVIEW_ATTEMPTS}회)에 도달하여 현재 초안으로 최종 작성합니다.")
        add_note(state, "[Review Draft] 재작성 한도 도달-현재 초안으로 확정")
        state['draft_feedback'] = None
        return state

    answer = input("이 초안으로 최종 보고서를 진행할까요? (y=확정 / n=수정 요청 / c=분석 취소): ").strip().lower()

    if answer == 'c':
        add_note(state, "[Review Draft] 사용자가 초안 검토 단계에서 분석을 취소함")
        raise AnalysisAborted("사용자가 초안 확인 단계에서 분석을 취소했습니다.")

    if answer == 'y':
        add_note(state, "[Review Draft] 사용자가 초안을 승인함")
        state['draft_feedback'] = None
        return state

    # 그 외 입력(기본값 n 취급): 수정 요청 받기
    feedback = input("수정하고 싶은 내용을 간단히 입력해주세요: ").strip()
...
# (신규) 초안 승인 여부에 따라 다음 노드를 결정하는 조건부 라우팅 함수
def route_after_review(state: CompanyState) -> str:
    if state.get('draft_feedback'):
        return 'Draft_Report' # 피드백이 있으면 초안 재작성으로 되돌아감
    return "Final_Report"       # 승인되었거나 한도 도달 시 최종 보고서로 진행
...
# 그래프 구성
# (Streamlit/웹 연동을 위해 checkpointer 파라미터만 추가. 기본값 None이면 기존 CLI 동작과 완전히 동일함)
def build_graph(checkpointer=None):
    ...
    graph.add_node("Review_Draft", Review_Draft)  # (신규) 초안 사용자 검토 노드 추가
    ...
    graph.add_edge("Draft_Report", "Review_Draft")  # (수정) 초안 작성 후 곧바로 최종화하지 않고 검토 단계로 이동

    # (신규) 조건부 라우팅: 사용자가 수정 요청을 남겼으면 Draft_Report로 되돌아가고,
    # 승인했거나 재작성 한도에 도달했으면 Final_Report로 진행
    ...
    graph.add_edge("Final_Report", "Report_wordfile_save")  # 그래프 연결
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # (신규) 사용자가 회사 확인/초안 검토 단계에서 취소를 선택하면
        # AnalysisAborted 예외가 발생하는데, 이걸 여기서 잡아서
        # 프로그램 전체가 죽지 않고 다음 회사명 입력으로 넘어가게 함
        try:
